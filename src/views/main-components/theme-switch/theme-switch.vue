            ...
            <DropdownMenu slot="list">
                <DropdownItem v-for="(item, index) in themeList" :key="index" :name="item.name">
                    <Row type="flex" justify="center" align="middle">
                        <span style="margin-right:10px;"><Icon :size="20" :type="item.name.substr(0, 1) !== 'b' ? 'md-happy' : 'ios-happy'" :color="item.menu"/></span>
...
export default {
    ...
    methods: {
        setTheme (themeFile) {
            ...
            let userName = this.$store.state.app.access_token;
    ...
    created () {
        ...
        let name = this.$store.state.app.access_token;
        if (localStorage.theme) {
