...
var chatUtils = {
	...
	regExpEmoji: function(message) {
		...
		for (var i = 0; i < chatUtils.emojiFaces.length; i++) {
			...
			// var pattern = new RegExp('\\[' + chatUtils.emojiFaces[i].en + '\\]', 'g');
	...
	sedLubMsg: function(content, type) {
		...
		if(!agentCloseChat){
			...
			if (!isSend || !content) return false;
			$('#lub_service_wrap .lub_emoji').hide();
			var str =
				'<li class="lub_chat_list chat_customer clearfix">\
				<div class="username"><p class="date">' +
				chatUtils.getNowFormatDate() +'</p></div>\
				<div class="chat_msg">\
			...
			if(content.indexOf('luban_img_') > -1){
				str =
					'<div class="chat_content">\
					  <a href="javascript:;" target="_blank" id='+ content +'>\<img src="'+ msg_config.pageUrl +'images/uploading.gif">\</a>\
					  </div>';
			}else{
				str =
				'<div class="chat_content">\
...
var phoneSystem = '', // 判断当前系统是ios还是Android
	appToken = msg_config.accessId,
	encrypt = msg_config.encrypt,
	parentUrl = msg_config.parentUrl, //上级页面
	fromUrl = msg_config.fromUrl, //来源页面
	seoKeyword = msg_config.seoKeyword, //seo关键字
	sessionId = chatUtils.getCookie('LUBAN_sessionid_'+appToken), //客户端的sessionid
	source = msg_config.source, //来源
	chat_log_page = 1,
	...
	socketNum = 1,            //连接次数
	agentCloseChat = false,   //客服关闭会话
	uploadNum = 1,            //上传文件id
	customerClose = false,    //顾客主动关闭会话
	isCloseSocket = false; 
...
$(document).on('change', '#lub_service_wrap .upload_image', function() {
	...
	uploadImg(msg_config.upload_image, file);
});
...
$(document).on('change', '#lub_service_wrap .upload_file', function() {
	...
	uploadFile(msg_config.upload_file, file);
});
...
var uploadFile = function(url, file) {
	if (file) {
		...
		xhr.open('POST', msg_config.lubanURL + url);
		xhr.send(fd);
...
// 获取父页面传过来的数据
function getMsgFromParent(data) {
	isSend = true;
	var msg = JSON.parse(data.data);
	// 初次进入页面
	if((msg.type && msg.type == "connect") || customerClose){
		initChat();
		return
	}else if(msg.type == "inviteCancel"){
		//拒绝邀请会话
		refuseInite();
		return
	}else if(isCloseSocket){
		websock('continue');
	} 
	...
	if (davData) {
		...
		if (davData && davData.windowType === 'common' && !chatUtils.isFromMobile()) {
			...
			// $('#lub_service_wrap .lub_warp').css({ width: '480px', float: 'left' });
			...
			if (davType == '1') {
				...
				davRight.style.display = 'none';
				davLeft.style.display = 'none';
			}
		} else {
		...
		if (davData.floatAdv && davData.windowType === 'common' && !chatUtils.isFromMobile()) {
			...
			var floatHight = $('.float_dav').outerHeight() + 20;
			$('#lub_service_wrap .lub_chat_content').css('top', '10px');
			$('#lub_service_wrap .lub_chat_wrap ul').css('margin-top', '40px');
			if (
		...
		if (
			...
			for (var i = 0; i < davData.advContent.length; i++) {
				...
				if (advItem.enable) {
					advlist +=
						'<li><a target="_blank" title="'+ advItem.advTitle +'" href=\'' + advItem.advUrl + "'>" + advItem.advTitle + '</a></li>';
				}
			}
		...
		if (msg.chatboxHeight && !chatUtils.isFromMobile() && davData.windowType != 'common') {
			var height = 450 + parseInt(msg.chatboxHeight) - 500 + 'px';
			// $('.lub_warp').css('height', height);
		}
	}
	// if (msg.companyName) {
...
var initChat = function(){
	var openlbUrl = msg_config.lubanURL + msg_config.init_lubankf;
	var data = {
		appToken: appToken,
		encrypt: encrypt,
		parentUrl: parentUrl,
	...
	chatUtils.ajaxRequest(openlbUrl, data, function(res) {
		if(res.code == 0){
			...
			chatUtils.setCookie('LUBAN_sessionid_' + appToken, res.data.sessionId);
			setTimeout(function() {
				websock();
			}, 1000);
...
var lubanSendChat = function(type, content, msgId) {
	var url = msg_config.lubanURL + msg_config.sendChatURL;
	var data = {
		sessionId: sessionId,
		appToken: appToken,
		encrypt: encrypt,
...
//拒绝邀请
var refuseInite = function(){
	var refuseUrl = msg_config.lubanURL + msg_config.refuseInite;
	var data = {
		appToken: appToken,
		encrypt: encrypt,
		sessionId: sessionId
	};
	chatUtils.ajaxRequest(refuseUrl, data, function(res) {
		window.parent && window.parent.postMessage && window.parent.postMessage('refuseInite', '*');
	})
}

var openLubanChat = function(type) {
	var webChat = msg_config.lubanURL + msg_config.webChat;
	var data = {
		appToken: appToken,
		encrypt: encrypt,
		sessionId: sessionId,
	...
	chatUtils.ajaxRequest(webChat, data, function(res) {
		if(res.code == 0){
			...
			$('.leave_msg').show();
			sessionId = res.data.sessionId;
			chatUtils.setCookie('LUBAN_sessionid', res.data.sessionId);
		}else if(res.code == 4444){
			$("#lub_davright").hide();
			$("#lub_davleft").hide();
			errorCode('抱歉，客服系统暂时无法使用');
...
var errorCode = function(msg){
	...
	<img src="'+ msg_config.pageUrl +'images/unusual.png">\
	<p>'+ mssg +'</p>\
	</div>';
...
var appendChatLog = function(d, type) {
	...
	if (data.length > 0) {
		for (var j = 0; j < data.length; j++) {
			...
			str +=
				'<div class="username"> <p>'+ (msg.fromUser == 1 ? msg.agent : '') +'</p><p class="date">' +
				msg.CreateTime +
				'</p></div>\
              <div class="chat_msg">\
			  <span class="angle"></span>';
...
var websock = function(type) {
	var wsuri = msg_config.socketUrl + '?sessionId=' + sessionId + '&appToken=' + appToken + '&encrypt=' + encrypt + '';
	...
	socket.onmessage = function(e) {
		...
		//留言
			...
			window.parent && window.parent.postMessage && (window.parent.postMessage('onmessage', '*'))
		}
...
var messageForesee = function (t,type) {
	...
	if((time-foreseeTime_old)>=t || !foreseeTime_old) {
		...
		var foreseeUrl = msg_config.lubanURL + msg_config.lubMsg_foresee;
		var data = {
			encrypt: encrypt,
			appToken: appToken,
			notice: msg,
...
var sendLeave = function(content, user, tel, email){
	var leaveUrl = msg_config.lubanURL + msg_config.leaveUrl;
	var data = {
		appToken: appToken,
		encrypt: encrypt,
		sessionId: sessionId,
...
//最小化
$('#lub_min').live('click', function() {
	window.parent && window.parent.postMessage && window.parent.postMessage('hideChatBox', '*');
});

//结束对话
$('#lub_close').live('click', function() {
	$('.lub_chat_close').show();
});
...
$('.lub_chat_close .sure_close').live('click', function() {
	...
	if (socket) {
		socket.close(3001, JSON.stringify({ s: sessionId, a: appToken }));
		clearTimeout(heartBeatTimeOut);
	}
});
...
$('#evaluation_btn').live('click',function(){
	var evaluationUrl = msg_config.lubanURL + msg_config.lub_evaluation;
	var data = {
		encrypt: encrypt,
		appToken: appToken,
		sessionId: sessionId,
	...
	chatUtils.ajaxRequest(evaluationUrl, data, function(res) {
		...
		chatUtils.appendLubInfo('<p>你给出了你给出了<span class="'+ str.label +'">'+ str.value +'</span></p>');
...
$(document).on('click','#lub_service_wrap .has_solve',function(){
	var solveUrl = msg_config.lubanURL + msg_config.lub_robotFeed;
		...
		if (items) {
			...
			Array.prototype.forEach.call(items, function (item) {
				...
				reader.onloadend = function (e) {
					...
					xhr.addEventListener('error', function(){ uploadImgFailed(event, id) }, false);
					xhr.addEventListener('abort', function(){ uploadImgCanceled(event, id) }, false);
					xhr.open('POST', msg_config.lubanURL + msg_config.upload_image);
					xhr.send(fd);
					xhr.getAllResponseHeaders()
